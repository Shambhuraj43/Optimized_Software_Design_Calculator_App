/*
Shambhuraj Wadghule
File name: Tree_Calculator.cpp
Project: Tree based calculator using software design principles.
Instructor: Dr. James Hill
Purpose: Provide the .cpp file for wrapper facade pattern for the calculator.
*/
...
Expr_Tree_Builder * Tree_Calculator::parse_expr (const std::string & infix, int level)
{
	...
	while(!input.eof())
	{
		...
		// COMMENT You should try the approach we discussed for parenthesis
		// in assignment 3 here. It should reduce the complexity of the code below.
		

		//Response: I had two diffent classes for open and closed parenthesis before a separate nodes.
		//But i was having some issues converting the expression fromm infix using those classes. Instead
		// of doing it that way, I included them in this class which can be the Wrapper class. So I am manually 		// checking the using the if condition if there is a parenthesis or not.                                    
			parLevel++;
			indexCount++;
	...
	while(infix_ != "QUIT")
	{
		...
		if(detected_ == 0)
		{
			...
			try{
				...
				if(details_)
					std::cout << "Resultant expression = \t" << res << "\n";
				else
					std::cout << "Resultant expression = \t" << res << "\n";
